...
export default async function handler(req, res) {
    ...
    switch (method) {
        ...
        case 'POST': // Create new user
            if (session) return res.status(307).redirect(`/api/user/${session.user.userId}`);
            const { email, password, firstName, lastName, contact, companyName, address, coreCategory, paidCategories } = req.body;
            ...
            var core_category_data = await executeQuery({
                query: "INSERT INTO user_categories(user_id, category_id, core) VALUES (?,?,1)",
                values: [userId, coreCategory]
            })
            
            paidCategories.forEach(cat => {
                executeQuery({
                    query: "INSERT INTO user_categories(user_id, category_id, core) VALUES (?,?,0)",
                    values: [userId, cat]
                })
            });
